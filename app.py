...
@app.route('/api/v2/playerStats/battingODI/all', methods=['GET'])
def get_all_battingstatsODI():
    with get_db_conn(dbname) as sqlite_conn:
        cur = sqlite_conn.cursor()
    cur.execute('''select * from Batting_Stats_Odi; ''')
...
@app.route('/api/v2/playerStats/bowlingODI/all', methods=['GET'])
def get_all_bowlerstatsODI():
    with get_db_conn(dbname) as sqlite_conn:
        cur = sqlite_conn.cursor()
    cur.execute('''select * from Bowling_Stats_Odi; ''')
...
@app.route('/api/v2/playerStats/bowlingODIPlot/all', methods=['GET'])
def get_all_bowlerstats_plot_ODI():
    with get_db_conn(dbname) as sqlite_conn:
        query = "select * from Bowling_Stats_Odi;"
        # Execute the query and fetch results into a pandas DataFrame
        df = pd.read_sql_query(query, sqlite_conn)
        ...
        # Save the plot as an image file
        plt.savefig('BowlerplotODI.png')  # Save the plot as 'plot.png' (you can choose any filename)

    # Return the image file in the API response
    return send_file('BowlerplotODI.png', mimetype='image/png')

@app.route('/api/v2/playerStats/battingODIPlot/all', methods=['GET'])
def get_all_battingstats_plot_ODI():
    with get_db_conn(dbname) as sqlite_conn:
        query = "select * from Batting_Stats_Odi;"
        # Execute the query and fetch results into a pandas DataFrame
        df = pd.read_sql_query(query, sqlite_conn)
        ...
        # Define the ticks with ranges for x and y axes
        plt.xticks(np.arange(0, max(x_values) + 1, 5))
        plt.yticks(np.arange(0, max(y_values) + 1, 50))

        # Save the plot as an image file
        plt.savefig('BattingPlotODI.png')  # Save the plot as 'BattingPlot.png' (you can choose any filename)

    # Return the image file in the API response
    return send_file('BattingPlotODI.png', mimetype='image/png')
